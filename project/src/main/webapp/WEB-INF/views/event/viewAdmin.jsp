		...
		<c:if test="${dto.e_result!=0}">
			<div align="center" style="margin-top: 20px;">
				...
				<c:if test="${dto.e_result>1}">
					신청 내역을 삭제하시겠습니까? 
					<button type="button" onclick="alert('삭제가 완료되었습니다.'); location.href='${path}/event/delete.do?e_num=${dto.e_num}&e_name=${dto.e_name}&e_email=${dto.e_email}'">확인</button>
				</c:if>
			</div>
		</c:if>
	</div>
...
더불어 반려 상태의 행사는 영업일 기준 7일 내 관리자가 삭제 처리할 수 있음을 알려드립니다.
